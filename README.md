...
### Sharing & Export
- **Shareable URLs** — camera position, color mode, and search query are all encoded in the URL so you can share exactly what you see
- **PNG snapshot** — download a high-resolution screenshot of the current view with one click
- **Copy link** — grab a permalink to the current view instantly
